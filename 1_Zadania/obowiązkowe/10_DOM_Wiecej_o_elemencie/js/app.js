document.addEventListener("DOMContentLoaded", function () {
    ...
    function getDatasInfo(y){
        ...
        for(var x=0;x<y.length;x++){
            tab.push(y[x].dataset.color); // każde li ze zmiennej links nie posiada atrybutu data, lecz dataset, z której wyciagam atrybut color
        }
        return tab;
    }
    ...
    for(i = 0; i < blocks.length; i++){
        ...
        // outerHTML ukazuje rodzica elementów innerHTML. outerHTML zawiera kontenery div oraz hiper łącza
        // innerHTML wypisuje same hiper łącza
    }
    for(i = 0; i < blocks.length; i++){
    ...
    for(i = 0; i < blocks.length; i++){
        console.log(blocks[i].innerHTML);
        console.log(blocks[i].outerHTML);
        // zamiast hiper łączy został w konsoli jak i na stronie
        // wypisany tekst "Nowa wartość diva o klasie blocks"
    }
